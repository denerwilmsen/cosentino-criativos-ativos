...
fetch("data.json").then((response) => { if (!response.ok) throw new Error('Dados indisponíveis'); return response.json(); }).then((payload) => { data = payload; render(); document.getElementById('meta-updated').textContent = `Atualizado em ${new Intl.DateTimeFormat('pt-BR', {dateStyle:'short',timeStyle:'short',timeZone:'America/Sao_Paulo'}).format(new Date(data.updatedAt))} · horário de Brasília · dia atual parcial`; }).catch(() => { document.getElementById("grid").innerHTML = '<div class="empty"><h3>Não foi possível carregar os criativos.</h3></div>'; });
